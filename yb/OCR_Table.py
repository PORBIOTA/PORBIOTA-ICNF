...
api=tesserocr.PyTessBaseAPI(psm=tesserocr.PSM.AUTO_OSD,lang='por',oem=tesserocr.OEM.LSTM_ONLY)

if len(sys.argv) > 1:
    file=sys.argv[1]
else:
    file='ocrtable-t1.jpg'
print("Using for input '%s'"%file)
    
if len(sys.argv) > 2:
    outputFile=sys.argv[2]
else:
    outputFile='%s.csv'%os.path.splitext(os.path.basename(file))[0]

print("Using for output '%s'"%outputFile)
...
#    for k in range(len(outer[i])):
#        t=outer[i][k]
#       print(t)
#        outer[i][k]= str(t).strip(' \x0c\n_')
#print(outer)
#exit()
arr = np.array(outer)
