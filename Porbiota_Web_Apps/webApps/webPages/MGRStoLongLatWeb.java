...
import helpers.MGRStoLongLat;

/**
 * Servlet implementation for the class MGRStoLongLat
 * 
 * @author Tomé Neves de Matos
 */
@MultipartConfig
@WebServlet(  name = "MGRS to Latitude Longitude",
urlPatterns = "/MGRStoLatLon", 
asyncSupported=true)
public class MGRStoLongLatWeb extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) 
		...
		final boolean ED50 = Boolean.valueOf(request.getParameter("ED50"));
		...
		if (fileName.contentEquals("")) {
			...
			request.getRequestDispatcher("/WEB-INF/MGRStoLatLon.jsp").forward(request, response);
		} else {
			...
			String outputFileName = "MGRSconverted_" +file.getName().substring(4,file.getName().length());
				...
				acontext.start(new Runnable() {
					...
					public void run() {
						...
						try {							
							...
							(new MGRStoLongLat()).convert(file.getAbsolutePath(), Databases.downloadsFolder.getAbsolutePath()+"/"+ outputFileName, zone, ED50,append);					
