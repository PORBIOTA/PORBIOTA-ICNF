...
import helpers.FixedColumnAdder;

/**
 * Servlet implementation for the class FixedColumnAdder
 * 
 * @author Tomé Neves de Matos
 */
@MultipartConfig
@WebServlet(  name = "Initial Column Adder",
urlPatterns = "/columnAdder", 
asyncSupported=true)
public class FixedColumnAdderWeb extends HttpServlet {

	private static final long serialVersionUID = -554280081451963056L;
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) 
		...
		values[0] = request.getParameter("collectionCode");
		values[1] = request.getParameter("type");
		values[2] = request.getParameter("modified");
		values[3] = "https://creativecommons.org/licenses/by/4.0/legalcode";
		values[4] = request.getParameter("institutionID");
		values[5] = request.getParameter("institutionCode");
		values[6] = request.getParameter("datasetName");
		values[7] = request.getParameter("basisOfRecord");
		values[8] =  "Present";
		...
		if (values[4].contentEquals("")) {
			values[4] = "53f347a7-99ce-4528-a085-5c76eb1301a0";
		}
		
		if (values[5].contentEquals("")) {
			values[5] = "ICNF";
		}
		
		if (values[7].contentEquals("")) {
			values[7] = "HumanObservation";
		}
		
		//Checks if no file was given
		if (fileName.contentEquals("")) {
			messages.put("name", String.format("Please select a file!"));	
			request.getRequestDispatcher("/WEB-INF/columnAdder.jsp").forward(request, response);
		} else {
			...
			String outputFileName = "Initial_" +file.getName().substring(4,file.getName().length());
				...
				acontext.start(new Runnable() {
					...
					public void run() {
						...
						try {							
							...
							FixedColumnAdder.addColumn(file.getAbsolutePath(), Databases.downloadsFolder.getAbsolutePath()+"/"+ outputFileName, values);					
