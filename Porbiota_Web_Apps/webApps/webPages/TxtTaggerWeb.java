...
import taggers.TxtTagger;
import taggers.TxtTagger.CustomTagsException;

/**
 * Servlet implementation for the class TxtTagger
 * 
 * @author Tomé Neves de Matos
 */
@MultipartConfig
@WebServlet(  name = "Txt Tagger",
urlPatterns = "/txtTagger", 
asyncSupported=true)
public class TxtTaggerWeb extends HttpServlet {

	private static final long serialVersionUID = -554280085451533056L;
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) 
		...
		//Get date type
		final String dateType = request.getParameter("dateType");	

		//Get custom tags
		final String customTagsInLine = request.getParameter("customTags");	

		//Gets only custom checkbox
		String s[] = request.getParameterValues("onlyCustom");
		Boolean onlyCustomValue = false;
		if (s != null && s.length != 0) {
			onlyCustomValue = true;
		}
		final boolean onlyCustom;
		if (onlyCustomValue){
			onlyCustom = true;
		}else { 
			onlyCustom = false;
		...
		if (fileName.contentEquals("")) {
			...
			request.getRequestDispatcher("/WEB-INF/TxtTagger.jsp").forward(request, response);
		} else {
			...
			String outputFileName = "Tagged_" +file.getName().substring(4,file.getName().length());
				...
				acontext.start(new Runnable() {
					...
					public void run() {
						...
						try {
							...
							(new TxtTagger()).tag(file.getAbsolutePath(), Databases.downloadsFolder.getAbsolutePath()+"/"+ outputFileName,dateType, customTagsInLine, onlyCustom);					
						} catch(CustomTagsException | PatternSyntaxException e) {
							...
							try (BufferedWriter writer = Files.newBufferedWriter(Paths.get(Databases.downloadsFolder.getAbsolutePath()+"/"+ outputFileName), StandardCharsets.UTF_8)) {
								writer.write("The custom tags were improperly defined!\n");
