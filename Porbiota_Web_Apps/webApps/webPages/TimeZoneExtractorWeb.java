...
import helpers.TimeZoneExtractor;

/**
 * Servlet implementation for the class TimeZoneExtractor
 * 
 * @author Tomé Neves de Matos
 */
@MultipartConfig
@WebServlet(  name = "Time Zone Extractor",
urlPatterns = "/timeZoneExtractor", 
asyncSupported=true)
public class TimeZoneExtractorWeb extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) 
		...
		if (fileName.contentEquals("")) {
			...
			request.getRequestDispatcher("/WEB-INF/TimeZoneExtractor.jsp").forward(request, response);
		} else {
			...
			String outputFileName = "TimeZone_" +file.getName().substring(4,file.getName().length());
				...
				acontext.start(new Runnable() {
					...
					public void run() {
						...
						try {
							...
							(new TimeZoneExtractor()).extract(file.getAbsolutePath(), Databases.downloadsFolder.getAbsolutePath()+"/"+ outputFileName);					
